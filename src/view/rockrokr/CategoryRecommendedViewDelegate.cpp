...
const static qreal S_BORDER_PEN_W = 1.0;

CategoryRecommendedViewDelegate::CategoryRecommendedViewDelegate(QObject *parent)
...
CategoryRecommendedViewDelegate::~CategoryRecommendedViewDelegate()
{

}

void CategoryRecommendedViewDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    const int margin = 2;
    const QSize coverSz(rect.width() - margin *2, rect.width() - margin *2);
...
//            qDebug()<<Q_FUNC_INFO<<" use default cover!!!";
    ...
    const int innerMargin = /*rect.height()*/innerSize.height()
                            - coverSz.height()
                            - MAIN_VIEW_HL_COVER_FONT_SIZE_PRIMARY
                            - MAIN_VIEW_HL_COVER_FONT_SIZE_SECONDARY
                            - _to_px(4);
    int textMargin = 0;
    if (innerMargin > 2) {
        textMargin = 2;
    ...
    // name
    {
        ...
        textTopLeft.setY(y);
        const QSizeF sf(textWidth, MAIN_VIEW_HL_COVER_FONT_SIZE_PRIMARY);
    ...
    // subname
    {
        ...
        const int y = textTopLeft.y() + MAIN_VIEW_HL_COVER_FONT_SIZE_PRIMARY + textMargin;
        textTopLeft.setY(y);
        const QSizeF sf(textWidth, MAIN_VIEW_HL_COVER_FONT_SIZE_SECONDARY);
        QRectF tf(textTopLeft, sf);
        QColor color;
        if (isHover) {
            color.setNamedColor("#FFFFFF");
...
QSize CategoryRecommendedViewDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    return QSize(MAIN_VIEW_HL_COVER_W, MAIN_VIEW_HL_COVER_H);
}

void CategoryRecommendedViewDelegate::initStyleOption(QStyleOptionViewItem *option, const QModelIndex &index) const
{
